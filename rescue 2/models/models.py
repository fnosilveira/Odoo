...
class Washings(models.Model):
	...
	# _inherit = 'em.laundry.other.than.wash' cambios Quimer
...
class SaleOrderExtend(models.Model):
	...
	state = fields.Selection(
		selection_add=[('draft',), ('order', 'Laundry Order'), ('process', 'Processing'), ('sale',), ('done',),
					   ...
					   ('cancel', 'Cancelled')])
	washing_count = fields.Integer('Ordenes de Trabajo', compute='get_washing_count')
	...
	def create(self, vals):
		if vals.get('name', _('New')) == _('New') and vals.get('is_laundry_order',False):
			...
			vals['name'] = self.env['ir.sequence'].next_by_code('sale.order.washings.sequence', sequence_date=seq_date) or _('New')
	...
	def _fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
		...
		if is_laundry_order:
			...
			for node in nodes:
				...
				node.set('domain', "[('type','in',['service','product','consu'])]")
	...
	def product_id_onchange(self):
		if self.order_id.is_laundry_order:
			...
			if self.product_id.other_charge_ids and self.product_id.type == 'service':
				wash_type_ids = self.product_id.other_charge_ids.mapped('other_work_id')
				if wash_type_ids:
					res = {
	...
	def wash_type_id_change(self):
		...
		for line in self.filtered(lambda l: l.order_id.is_laundry_order):
			for other_than_wash_id in line.other_than_wash_ids:
				work_charge = sum(oher.price for oher in self.product_id.other_charge_ids.filtered(lambda l:l.other_work_id.id == other_than_wash_id._origin.id))
				if not work_charge:
