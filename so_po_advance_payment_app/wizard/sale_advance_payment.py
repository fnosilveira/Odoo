...
class SaleAdvancePayment(models.TransientModel):
	...
	# def _domain_caja(self):
	# 	ids_caja=[]		
	# 	objeto_movimiento_caja = self.env['movimiento.caja.chica'].search([('state','=', 'abierto'),('user_id','=', self.env.user.id)])
	# 	for movimiento in objeto_movimiento_caja:			
	# 		ids_caja.append(movimiento.caja_chica_id.id)
	# 	return ids_caja

	# def _default_caja(self):
	# 	id_caja= False	
	# 	objeto_movimiento_caja = self.env['movimiento.caja.chica'].search([('state','=', 'abierto'),('user_id','=', self.env.user.id)],limit=1)					
	# 	id_caja = objeto_movimiento_caja.caja_chica_id.id
	# 	return id_caja
	...
	# caja_chica_id = fields.Many2one('caja.chica',default=_default_caja, domain=lambda self: [('id', 'in', self._domain_caja())])
	...
	def gen_sale_advance_payment(self):
		for record in self:
			...
			payment_data = {
				...
				# 'caja_chica_id' : self.caja_chica_id.id,
			}
